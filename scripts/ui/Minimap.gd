...
func _draw() -> void:
	...
	# 3. Vintage Compass Rose (centered with soft archival transparency)
	if COMPASS_TEX:
	...
	# 4. Inked Battlefield Entity Blips
	_draw_group("bunker", Color(0.55, 0.18, 0.65), center, 3.5)
	_draw_group("emplacement", Color(0.55, 0.18, 0.65), center, 3.5)
	_draw_group("vehicle", Color(0.92, 0.52, 0.12), center, 3.5, true)
	_draw_group("enemies", Color(0.85, 0.15, 0.15), center, 3.0)
...
func _draw_group(group: String, color: Color, center: Vector2, blip_radius: float, is_box: bool = false) -> void:
	for node in get_tree().get_nodes_in_group(group):
		...
		if offset.length() > radar_range:
			continue
		var mapped: Vector2 = center + (offset / radar_range) * (radar_size * 0.44)
		if is_box:
